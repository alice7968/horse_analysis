...
RACR_URL_DIR = PARENTDIR + "/race_url"
RACR_HTML_DIR = PARENTDIR + "/race_html"
CSV_DIR = PARENTDIR + "/csv"
...
def make_csv_from_html():
    for year in range(2020, now_datetime.year+1):
                ...
                for file_name in file_list:
                    with open(html_dir+"/"+file_name, "r") as f:
                        ...
                        for horse_list in horse_list_list:
                            ...
                            horse_se = horse_se.to_frame().T
                            horse_df = pd.concat([horse_df,horse_se],ignore_index=True)
                            race_se = pd.Series(race_list, index=race_df.columns)
                            race_se = race_se.to_frame().T
                            race_df = pd.concat([race_df,race_se],ignore_index=True)
                        
                                    
        race_df.to_csv(save_race_csv, header=True, index=False, encoding="utf-8-sig")
        horse_df.to_csv(horse_race_csv, header=True, index=False, encoding="utf-8-sig")
...
def get_rade_and_horse_data_by_html(race_id, html):
    ...
    try:
        ...
        race_datails1 = data_intro.find("span").get_text().strip("\n").split("\xa0/\xa0")#ちょい変更しましたp->span
    
        race_list.append(race_datails1[0]) # race_course
        race_list.append(race_datails1[1]) # weather
        race_list.append(race_datails1[2]) # ground_status
        race_list.append(race_datails1[3]) # time
        race_datails2 = data_intro.find("p", class_="smalltxt").get_text().strip("\n").split(" ")
        race_list.append(race_datails2[0]) # date
        race_list.append(race_datails2[1]) # where_racecourse
        ...
        for rank in range(1, len(result_rows)):
            ...
            # time_value(premium)
            #horse_list.append(result_row[9].get_text())#なし
            # half_way_rank
            ...
            # tame_time(premium)
            #horse_list.append(result_row[15].get_text())#なし
            # 16:コメント、17:備考
            # tamer_id
    ...
    except: #race_htmlが存在しない場合
        print("race html not found: ",race_id)
        horse_list = [race_id]
        for _ in range(27):
            race_list.append("")
        for _ in range(16):#2項目premium限定なので消しています
            horse_list.append("")
        horse_list_list.append(horse_list)
